...
import flightapp.wrapper.TestConnectionWrapper;
...
public class FlightServiceTest {
  ...
  public static void initCredential() {
    ...
    if (credentialServerPort != null) {
      ...
      try {
        System.out.println(String.format("Trying to connect to '%s' to receive database credential......", credentialServerPort));
  ...
  public static void setup() {
    ...
    initCredential();
    try {
      Connection conn = serverURL == null ? QueryAbstract.openConnectionFromDbConn()
          : QueryAbstract.openConnectionFromCredential(serverURL, dbName, adminName, password);
  ...
  public static Query createTestQuery() throws SQLException, IOException {
    Connection conn = serverURL == null ? QueryAbstract.openConnectionFromDbConn() :
        QueryAbstract.openConnectionFromCredential(serverURL, dbName, adminName, password);
    TestConnectionWrapper testConn = new TestConnectionWrapper(conn);
    Query query = new Query(testConn);
    testConn.setAllowCreatePreparedStatement(false);
    return query;
  }
  ...
  public void runTest() throws Exception {
    ...
    try {
      ...
      if (System.getProperty("public_test_only") != null && !PUBLIC_TEST_LIST.contains(file)) {
        error_message = String.format("Failed: %s. No output since this test is private.",
            this.file);
      } else {
        error_message = String.format("Failed: %s. Actual outcome were: \n%s\n\nPossible outcomes were: \n%s\n",
